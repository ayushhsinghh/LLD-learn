...
export function MovieClassBlueprint() {
  return <section className="flex h-full min-h-0 flex-col rounded-xl border bg-[var(--paper-2)] p-3"><Tabs defaultValue="values" className="flex min-h-0 flex-1 flex-col"><div className="overflow-x-auto"><TabsList className="grid min-w-max grid-flow-col">{movieBookingClassDiagrams.map((tab) => <TabsTrigger key={tab.id} value={tab.id} className="px-3 py-1.5 text-[9px]">{tab.label}</TabsTrigger>)}</TabsList></div>{movieBookingClassDiagrams.map((tab) => <TabsContent key={tab.id} value={tab.id} className="mt-2 min-h-0 flex-1"><article className="grid h-full min-h-0 grid-rows-[minmax(0,1fr)_auto] overflow-hidden rounded-lg border bg-white"><div className="flex min-h-0 items-center justify-center overflow-hidden bg-[#fbf7ef] p-1"><Image src={tab.image} alt={tab.alt} width={tab.width ?? 1536} height={tab.height ?? 1024} className="h-full w-full object-contain" unoptimized /></div><div className="grid gap-1 border-t px-3 py-2 sm:grid-cols-2"><p className="text-[8px] leading-3 text-[var(--muted)]"><strong>Read it: </strong>{tab.reading}</p><p className="text-[8px] leading-3 text-[var(--muted)]"><strong className="text-[var(--accent-dark)]">Principle: </strong>{tab.principle}</p></div></article></TabsContent>)}</Tabs></section>;
}
